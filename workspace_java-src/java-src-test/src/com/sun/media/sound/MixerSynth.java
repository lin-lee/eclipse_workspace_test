    ...
    */
	...
	protected void implSend(MidiMessage message, long timeStamp) {
		...
		throw new IllegalStateException("Synthesizer is not open.");
            ...
            /* process only short messages (see ShortMessage class description),
               but there is no requirement for message to be
               of ShortMessage class - fix for 6186488 */
            int msgLength = message.getLength();
            if (msgLength > 0 && msgLength < 4)
            {
                byte[] msgData = message.getMessage();
                if (!parse(msgData[0] & 0xF0, msgData[0] & 0x0F,
                            (msgLength > 1) ? (msgData[1] & 0xFF) : 0,
                            (msgLength > 2) ? (msgData[2] & 0xFF) : 0)) {
                    if (Printer.err)
                        Printer.err("Message parsing error: " + message);
                }
            } else {
                if (Printer.err)
